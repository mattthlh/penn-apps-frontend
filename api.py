...
movies = pd.read_csv('movies_summaries.csv', usecols=['title', 'genres', 'summary'])
...
def pick_movie():
    ...
    if pref_num != 0:
        has_no_first = not(movies.genres[index_pick].__contains__(user_preferences[0]))
        has_no_second = not(movies.genres[index_pick].__contains__(user_preferences[1]))
        has_no_third = not(movies.genres[index_pick].__contains__(user_preferences[2]))

        while ((has_no_first and user_preferences[0] != 'none') or (has_no_second and user_preferences[1] != 'none') or ((
                has_no_third and user_preferences[2] != 'none') and (random.random() > 0.6))) or (index_pick in done_indexes):
            index_pick = random.randint(0, len(movies) - 1)
